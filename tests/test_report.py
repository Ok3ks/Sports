...
class TestLeagueWeeklyReport():
    ...
    def test_weekly_score_transformation(self,league_weekly_score):
        ...
        new_cols = set(['rank', 'entry_id', 'points_breakdown', 'captain_points', 'vice_captain_points'])

        assert new_cols.intersection(cols) == new_cols, f'Add columns {new_cols.difference(new_cols.intersection(cols))}'
    ...
    def test_merge_league_weekly_transfer(self, league_weekly_score, league_weekly_transfer):
        ...
        assert new_transfer_cols.intersection(cols) == new_transfer_cols, f'Add columns {new_transfer_cols.difference(new_transfer_cols.intersection(cols))}'
    ...
    def test_add_auto_sub(self, auto_sub_fixture):
        ...
        test.add_auto_sub()
        auto_sub_cols = set(['auto_sub_in_player', 'auto_sub_out_player', 'auto_sub_in_points', 'auto_sub_out_points'])

        cols = test.f.columns
        assert auto_sub_cols.intersection(cols) == auto_sub_cols, f'Add columns {auto_sub_cols.difference(auto_sub_cols.intersection(cols))}'
