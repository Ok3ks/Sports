...
class ParticipantReport(Participant):
    ...
    def weekly_score_transformation(self):
        ...
        self.o_df['points_breakdown'] = [[get_player_stats_from_db(y, event)[0] for y in self.o_df['players'][event-1].split(",")] for event in range(1, self.gw+1)]
        self.o_df['captain_points'] = [get_player_stats_from_db(self.o_df['captain'][event-1], event)[0]*2  for event in range(1, self.gw+1)]
        self.o_df['vice_captain_points'] = [get_player_stats_from_db(self.o_df['vice_captain'][event-1], event)[0]  for event in range(1, self.gw+1)]
    ...
    def merge_league_weekly_transfer(self):
        ...
        self.f['transfer_points_in'] = [sum([get_player_stats_from_db(y, event)[0] for y in self.f['element_in'][event]]) for event in range(1, self.gw+1)]
        #self.f['transfer_points_in'] = self.f['element_in'].map(lambda x: sum([get_player_stats_from_db(y, self.gw)[0] for y in x]))
        self.f['transfer_points_out'] = [sum([get_player_stats_from_db(y, event)[0] for y in self.f['element_out'][event]]) for event in range(1, self.gw+1)]
    ...
    def add_auto_sub(self):
        ...
        self.f['auto_sub_in_points'] = [sum([get_player_stats_from_db(y, event)[0] for y in self.f['auto_sub_in_player'][event-1]]) for event in range(1, self.gw+1)]
    ...
    def plots_1(self):
        ...
        points_on_bench =  so.Plot(self.f, y = 'points_on_bench',  x="gw")\
                    .add(so.Lines())\
    ...
    def create_report(self,fp,gw):
        ...
        def promoted_vice():
            ...
            for row in self.o_df.itertuples():
                if check_minutes(int(row.captain), row.gw)[0] == 0:
                    ...
                    self.vice_to_cap[get_player(row.vice_captain)].append(get_player_stats_from_db(row.vice_captain, row.gw)[0]*2)
        ...
        def outliers():
            ...
            return {"exceptional": exceptional ,"abysmal": abysmal, "season_average": season_average}
#       
...
#
        def worst_transfer_in_week():
            ...
            for i in range(1,3):
                ...
                worst_transfer_in.append((gw, get_player(id =player_in), get_player(id =player_out), points_lost))
        ...
        def best_transfer_in_week():
            ...
            for i in range(0,3):
                ...
                best_transfer_in.append((gw, get_player(id =player_in), get_player(id =player_out), points_gained))
        ...
        def luckiest_week():
            ...
            for i in range(3):
                ...
                lucky_week.append((gw, get_player(id =auto_sub_in), get_player(id =auto_sub_out), auto_sub_points))
        ...
        def most_points_on_bench():
            ...
            for i in range(3):
                ...
                point_player = {get_player(id =i):get_player_stats_from_db(int(i), gw)[0] for i in player_on_bench}
        ...
        def highest_point() -> list :
            ...
            highest_point = [(highest_point['gw'].tolist()[0]), highest_point['total_points'].tolist()[0]]
            return {"highest_point": highest_point}

        def lowest_point()-> list:
            lowest_point = self.f[self.f['total_points'] == min(self.f['total_points'])]
            lowest_point =[(lowest_point['gw'].tolist()[0]), lowest_point['total_points'].tolist()[0]]
            return {"lowest_point": lowest_point}

        self.captain = captain()
        self.vice_to_cap = promoted_vice()
        output = {"captain": self.captain, "promoted_vice": self.vice_to_cap, "used_chips": [x for x in self.used_chips.tolist() if x != None] }
...
if __name__ == "__main__":
    ...
    parser.add_argument('-g', '--gameweek', type= int, default= get_curr_event()[0] ,help = "Gameweek you are trying to get a report of")
    parser.add_argument('-l', '--entry_id', type= int, required=True, help = "ID of pkayer you're interested in ")
    ...
    test.create_report(f"{FPL_WRAP_DIR}/{str(args.entry_id)}/{str(args.entry_id)}_{str(args.gameweek)}.json", args.entry_id)  #Move to S3
    test.plots_2()
