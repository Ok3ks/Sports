...
with open('json/downtown_players.json', 'r') as ins:
    participant_json = json.load(ins)  #store as json because it can change weekly or update table weekly
...
def text_output():
    ...
    for key,values in obj.items():
        ...
        if key == "most_points":
            for value in values:
                print(f"{participant_json[value[0]]} was unlucky, with {value[2]} points on the bench")
                #add more context to output

        if key == "jammy_points":
            print(f"\nLucky players rescued by Auto_sub")
            for value in values:
                print(f"{participant_json[value[0]]}  {value[1]} coming on for {value[2]}, points_gained {value[3]}")
