...
if __name__ == "__main__":
    ...
    for n in range(0, last_element, 100):
        #optimum number of spawned threads to 100
        req = [gevent.spawn(get_gw_transfers_mt, all = True, gw = 8, entry_id = i) for i in range(n, n+100, 1)]
        ...
        #except AttributeError:
            #time.sleep(30)
            #req = [gevent.spawn(get_gw_transfers, all = True, entry_id = i) for i in range(n, n+100, 1)]
            #res = [response.value for response in gevent.iwait(req)]
            #df = pd.DataFrame(res)
        #finally:
            #print(df)
